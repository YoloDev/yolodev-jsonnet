...
use crate::{
  ast::full::SpanBuilder,
  lex::{
    span::Span,
    token::{private, Spanned, Tok},
  },
  parse::{error::Result, Parse, ParseStream},
};
use alloc::vec;
use core::{
  fmt,
...
pub struct Punctuated<T, P> {
  inner: SmallVec<[(T, P); 4]>,
  last: Option<Box<T>>,
}

impl<T: Spanned, P: Spanned> private::Sealed for Punctuated<T, P> {}
impl<T: Spanned, P: Spanned> Spanned for Punctuated<T, P> {
  fn span(&self) -> Span {
    let mut builder = SpanBuilder::new();
    for (tok, punct) in &self.inner {
      builder = builder.add(tok).add(punct);
    }

    match &self.last {
      Some(b) => builder = builder.add(b.as_ref()),
      None => (),
    }
...
impl<T: Spanned, P: Spanned> Index<usize> for Punctuated<T, P> {
  type Output = T;
...
impl<T: Spanned, P: Spanned> IndexMut<usize> for Punctuated<T, P> {
