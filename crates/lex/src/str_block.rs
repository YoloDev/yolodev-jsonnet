...
pub(super) enum StringBlockToken {
  Valid,
...
pub(super) fn lex_str_block<'a>(lex: &mut logos::Lexer<'a, RawToken>) -> StringBlockToken {
  struct Context<'a> {
    source: &'a str,
    index: usize,
  ...
  fn guess_token_end_and_bump<'a>(lex: &mut logos::Lexer<'a, RawToken>, ctx: &Context<'a>) {
    let end_index = ctx
      .rest()
      .find("|||")
      .map(|v| v + 3)
